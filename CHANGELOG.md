...
- **README "Migration from 0.9.x" expansion**: previous note only covered the `list_*` envelope change. Now enumerates all 5 shape-change categories explicitly, including `push_files` (array → envelope), `get_repository_tree` (two-item pair → envelope), the full set of 8 wiki tools that lost their human-readable prefix via `formatWikiPageResponse` / `formatWikiAttachmentResponse` (`get`/`create`/`edit` × `project`/`group` × `wiki_page` + `upload_project_wiki_attachment` + `upload_group_wiki_attachment`), and the unchanged status / raw-blob tools. Closes codex P2 on PR #103 (review on `437b046`) and codex P2 round-4 on `8e97867` (which caught the under-documented wiki-tool scope: I'd named 2 shorthand non-existent tools instead of the 8 real ones).
